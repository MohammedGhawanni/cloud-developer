...
(async () => {
  ...
  app.get('/cars/make/:make', (req: Request, res: Response) => {
      let { make } = req.query;
      ...
      let cars_make_list = cars;
      cars_make_list = cars_list.filter((element) => {
  ...
  app.post('/cars', (req:Request, res:Response) => {
    ...
    if(!make || !type || !model || !cost || !id){
      res.status(400).send(`make, type, model, cost, id are required`);
    }
